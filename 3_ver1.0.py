...
if not st.session_state['logged_in']:
    ...
    with main:
        ...
        if st.session_state['menu_mode'] == "Knowledge Graph":
            ...
            # ==========================================
            # 💧 WATER DROPLET CONFIG (Based on Default)
            # ==========================================
            # 기본 솔버(barnesHut)를 쓰되, 물방울처럼 탄성을 가짐
            cfg = Config(
                width="100%", 
                height=600, 
                directed=False,
                ...
                physics={
                    "enabled": True,
                    "solver": "barnesHut", # [중요] Default에서 잘 작동했던 솔버
                    
                    "barnesHut": {
                        "gravitationalConstant": -2000,
                        "centralGravity": 0.3,
                        "springLength": 95,
                        "springConstant": 0.02, # [Tweak] 탄성계수 낮춤 (느슨한 스프링)
                        "damping": 0.09,        # [Tweak] 저항
                        "avoidOverlap": 0.2
                    },
                    
                    # [핵심] False로 두어 로딩 과정을 애니메이션화
                    "stabilization": {
                        "enabled": False, 
                    },
