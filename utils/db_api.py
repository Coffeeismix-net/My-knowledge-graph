...
def load_valuechains():
    ...
    try:
        ...
        ws_chunks = get_or_create_sheet(wb, "valuechain_chunks", ["id", "index", "content"])
        ...
        for row in meta_data:
            doc_id = str(row['id'])
            full_json = "".join(content_map.get(doc_id, []))
            result.append({
                "id": doc_id,
                "title": row['title'],
                "json_data": full_json,
...
def add_valuechain(title, json_str):
    wb = get_workbook()
    ...
    try:
        ...
        ws_chunks = get_or_create_sheet(wb, "valuechain_chunks", ["id", "index", "content"])
        chunks = chunk_text(json_str)
        chunk_rows = [[new_id, i, c] for i, c in enumerate(chunks)]
        ws_chunks.append_rows(chunk_rows)
        
        return {"id": new_id, "title": title, "json_data": json_str, "created_at": now_str}
    except: return None
...
# ==========================================
# (기존 load_stocks, add_stock 등 Stock/Node 관련 코드는 그대로 유지해주세요. 
#  지면 관계상 여기서는 생략하지 않고, 파트너님의 기존 파일 내용을 그대로 두시면 됩니다.
#  단, get_kst_now_str Helper 함수가 생겼으니 기존 코드에서도 이를 활용하면 좋습니다.)

# ... (아래는 기존 DB API 코드 복사해서 붙여넣으세요. 단, import 문과 Helper 함수는 위 코드로 교체) ...
# (편의를 위해 Stock 관련 코드도 다시 적어드립니다. Node/Settings 부분도 꼭 유지하세요)
        ...
        for row in data_rows:
            ...
            if kw_raw:
                ...
                for cand in candidates:
                    if not bool(re.search(r'\d{2,4}[-.]\d{1,2}[-.]\d{1,2}', str(cand))): real_kws.append(cand)
                    else:
                        if not created_at: created_at = cand
            
            stocks.append({
                "id": doc_id, "company": company, "title": title,
                "content": "".join(content_map.get(doc_id, [])), "keywords": real_kws, "created_at": created_at
            })
        return stocks
    except: return []
...
def move_stock_to_trash(doc_data):
    ...
    try:
        ...
        all_vals = ws_chunks.get_all_values()
        if len(all_vals) > 1:
            kept_rows = [r for r in all_vals[1:] if str(r[0]) != str(doc_data['id'])]
            ws_chunks.clear()
            ws_chunks.append_row(all_vals[0])
    ...
    try:
        ...
        for row in meta_data:
            ...
            trash_list.append({
                ...
                "content": "".join(content_map.get(doc_id, [])), "keywords": kws,
                "created_at": str(row['created_at']), "deleted_at": str(row['deleted_at'])
            })
        return trash_list
    except: return []
...
# ... (Node, Settings, AI, Color 관련 함수는 이전과 동일하게 유지) ...
